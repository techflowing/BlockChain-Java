...
import win.techflowing.blockchain.block.BlockChainUtils;
...
public class CLI {
    ...
    private void createBlockchain(String address) throws Exception {
        BlockChainUtils.createBlockChain(address);
    ...
    private void getBalance(String address) throws Exception {
        ...
        BlockChain blockchain = BlockChainUtils.createBlockChain(address);
        // 得到公钥Hash值
        byte[] versionedPayload = Base58Check.base58ToBytes(address);
        byte[] pubKeyHash = Arrays.copyOfRange(versionedPayload, 1, versionedPayload.length);

        TransOutput[] TransOutputs = blockchain.findUTXO(pubKeyHash);
        int balance = 0;
        if (TransOutputs != null && TransOutputs.length > 0) {
            for (TransOutput TransOutput : TransOutputs) {
                balance += TransOutput.getValue();
            }
        }
    ...
    private void send(String from, String to, int amount) throws Exception {
        ...
        BlockChain blockchain = BlockChainUtils.createBlockChain(from);
    ...
    private void printChain() throws Exception {
        BlockChain blockchain = BlockChainUtils.initBlockchainFromDB();
        for (BlockChain.BlockChainIterator iterator = blockchain.getBlockChainIterator(); iterator.hashNext(); ) {
            Block block = iterator.next();
            if (block != null) {
                boolean validate = ProofUtils.createProofOfWork(block).validate();
